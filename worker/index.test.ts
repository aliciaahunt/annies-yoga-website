...
describe('form Worker', () => {
  it('verifies and forwards an enquiry without exposing either secret', async () => {
    ...
    const delivery = fetchMock.mock.calls[1][1].body as FormData
    expect(Object.fromEntries(delivery.entries())).toEqual({
      access_key: 'web3forms-key',
      email: 'alex@example.com',
      enquiry_type: 'Retreats',
      from_name: "Annie's Yoga website",
      message: 'When is the next retreat?',
      name: 'Alex Murphy',
  ...
  it('forwards the complete reservation contract and preserves its subject', async () => {
    ...
    expect(response.status).toBe(200)
    const delivery = fetchMock.mock.calls[1][1].body as FormData
    expect(delivery.get('subject')).toBe('New class reservation request: Yoga — Monday 17 August 2026')
    expect(delivery.get('class_location')).toBe('Castlederg')
  })
