...
describe('SchedulePage reservations', () => {
  ...
  it('shows all seven days on a proportional time grid with consistent class cards', () => {
    ...
    expect(within(yogaPricing).getByText('£13')).toBeInTheDocument()
    expect(within(yogaPricing).getByText('£65')).toBeInTheDocument()
    ...
    expect(pricing).toHaveTextContent("Use within 6 weeks of Annie's confirmation email")
  ...
  it('opens a reservation form with the selected class and pricing information', async () => {
    ...
    expect(within(dialog).getByLabelText('Class pricing')).toHaveTextContent('£13 per class')
    ...
    expect(message).toHaveAccessibleDescription(/tell Annie if you’d like to arrange the £65 six-week package or if this booking is one of your existing six classes/)
  })
